...
class ListOfExtraServices extends StatelessWidget {
  const ListOfExtraServices({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: GridView.builder(
          ...
          itemCount: dataRoom_Controller.extra.length,
          scrollDirection: Axis.vertical,
          itemBuilder: (context, index) {
            return Service(
              selectCat: index,
              extraModel: ExtraModel.fromJson(dataRoom_Controller.extra[index]),
            );
          }),
    );
  }
